...
const demandTypesByCategory={
  客戶:['客戶預測','客戶PO','緊急詢單'],
  內部:['內部預測','內部需求','計畫庫存']
};
const sourcePeopleMaster={
  客戶:[...new Map(['客戶預測','緊急詢單'].flatMap(type=>demandProposers[type]||[]).map(pair=>[pair[0],pair])).values()],
  內部:[...new Map(['內部預測','內部需求','計畫庫存'].flatMap(type=>demandProposers[type]||[]).map(pair=>[pair[0],pair])).values()]
};
const internalDemandPurposes=['S&OP內部需求預測','經核准的內部補庫／調撥需求','區域成品安全庫存'];
function sourceCategoryFor(type){return ['客戶預測','客戶PO','緊急詢單'].includes(type)?'客戶':'內部';}
...
function statusForDemand(type,sku,demandDate){
  ...
  if(type==='緊急詢單')return {status:'詢單評估',requiresApproval:false,...assessment};
  if(type==='內部預測')return {status:'待整合',requiresApproval:false,...assessment};
  if(type==='內部需求')return {status:'待確認',requiresApproval:false,...assessment};
  return {status:'已確認',requiresApproval:false,...assessment};
}
...
const state={
  ...
  events:loadStored('ct-events-v6-lt',structuredClone(baseEvents)),
  demandEvents:loadStored('ct-demand-events-v6-lt',structuredClone(baseDemandEvents)),
...
const sourceClass={'客戶':'success','內部':'neutral','客戶預測':'info','內部預測':'neutral','內部需求':'info','客戶PO':'success','計畫庫存':'warning','緊急詢單':'danger'};
...
function setupInteractions(){
  ...
  $('#resetDemo').addEventListener('click',()=>{
    clearStored(['ct-events-v3','ct-events-v4','ct-events-v5-empty','ct-events-v6-lt','ct-demand-events-v1','ct-demand-events-v2','ct-demand-events-v3','ct-demand-events-v5-empty-select','ct-demand-events-v6-lt']);
...
const validDemandTypes=['客戶預測','客戶PO','緊急詢單','內部預測','內部需求','計畫庫存'];
function defaultPurpose(source,audience=''){
  if(source==='客戶預測')return '客戶預測';
  ...
  if(source==='緊急詢單')return '交期與數量可行性詢單';
  return '正式客戶訂單';
}
...
function resolveDemandSource(raw){
  const rawSource=String(raw.source||raw.sourceCategory||'').trim();
  const rawType=String(raw.demandType||raw.type||'').trim();
  if(demandSourceCategories.includes(rawSource)){
    const type=rawType||demandTypesByCategory[rawSource][0];
    if(!demandTypesByCategory[rawSource].includes(type))throw new Error(`${rawSource}來源不支援需求類型：${type}`);
    return {sourceCategory:rawSource,demandType:type};
  }
  if(validDemandTypes.includes(rawSource))return {sourceCategory:sourceCategoryFor(rawSource),demandType:rawSource};
  throw new Error(`無效需求來源：${rawSource||'未填'}`);
}
function createImportedDemandEvent(raw){
  ...
  const assessment=statusForDemand(source,raw.sku,demandDate);
  ...
  const customer=sourceCategory==='客戶'?String(raw.customer||raw.audience||'待選客戶').trim():'';
  const internalPurpose=sourceCategory==='內部'?String(raw.audience||defaultPurpose(source,'')).trim():'';
  const event={id,batchId:`${sourceBatchPrefix(source)}-${raw.factory}-${demandDate.replaceAll('-','')}-${String((seed%999)+1).padStart(3,'0')}`,
    ...
    purpose:sourceCategory==='客戶'?defaultPurpose(source,''):internalPurpose,customer:source==='客戶PO'?customer:'',destinationRegion:'',destinationCountry:source==='客戶PO'?dest.country:'',destinationCity:source==='客戶PO'?dest.city:'',
    prospectCustomer:['客戶預測','緊急詢單'].includes(source)?customer:'',prospectRegion:'',prospectCountry:['客戶預測','緊急詢單'].includes(source)?dest.country:'',prospectCity:['客戶預測','緊急詢單'].includes(source)?dest.city:'',
    soNo:'',shippingStatus:'',incoterm:'',transportMode:'',note:'',
...
function syncManualDepartmentFromProposer(){
  const category=$('#manualDemandSource')?.value||'客戶';const proposer=$('#manualDemandProposer')?.value;const pair=(sourcePeopleMaster[category]||[]).find(item=>item[0]===proposer);
...
function renderManualSourceOptions(reset=false){
  ...
  if(category==='客戶'){
    audienceSelect.innerHTML=selectOptionsHtml(customers.map(x=>x.name));destinationSelect.innerHTML=selectOptionsHtml(customers.map(x=>`${x.country}／${x.city}`));
    if(customers.some(x=>x.name===keepAudience))audienceSelect.value=keepAudience;if([...destinationSelect.options].some(x=>x.value===keepDestination))destinationSelect.value=keepDestination;
...
function setupDemandImport(){
  ...
  $('#manualDemandSource').addEventListener('change',()=>{renderManualSourceOptions(true);renderManualDemandSummary();});
  $('#manualDemandType').addEventListener('change',renderManualDemandSummary);
  $('#manualDemandProposer').addEventListener('change',()=>{syncManualDepartmentFromProposer();renderManualDemandSummary();});
  ['manualDemandFactory','manualDemandQty','manualDemandDate','manualDemandAudience','manualDemandDestination'].forEach(id=>$('#'+id)?.addEventListener('change',renderManualDemandSummary));
  ...
  $('#resetManualDemand').addEventListener('click',()=>setTimeout(()=>{sku.selectedIndex=0;$('#manualDemandSource').value='客戶';renderManualFactoryOptions();renderManualSourceOptions(true);$('#manualDemandQty').value='5000';$('#manualDemandDate').value=targetDate;renderManualDemandSummary();},0));
...
function renderManualDemandSummary(){
  const root=$('#manualDemandSummary');if(!root)return;const item=skuLookup[$('#manualDemandSku')?.value];const factory=$('#manualDemandFactory')?.value;const qty=Number($('#manualDemandQty')?.value||0);const category=$('#manualDemandSource')?.value;const type=$('#manualDemandType')?.value;const demandDate=$('#manualDemandDate')?.value;
  if(!item){root.innerHTML='';return;}const assessment=demandDate?statusForDemand(type,item.sku,demandDate):null;
  const chain=type==='緊急詢單'?'緊急詢單 → 客戶PO（獨立鏈）':type==='客戶預測'?'客戶預測 → 客戶PO':type==='內部預測'?'內部預測 → 內部需求 → 計畫庫存':type==='內部需求'?'內部需求 → 計畫庫存':type==='計畫庫存'?'計畫庫存 → 客戶PO消耗':'直接客戶PO';
...
function saveManualDemand(){
  ...
  try{const category=$('#manualDemandSource').value;const type=$('#manualDemandType').value;const event=createImportedDemandEvent({source:category,demandType:type,sku:$('#manualDemandSku').value,factory,qty:$('#manualDemandQty').value,demandDate:$('#manualDemandDate').value,proposer:$('#manualDemandProposer').value,proposerDept:$('#manualDemandDept').value,audience:$('#manualDemandAudience').value,customer:category==='客戶'?$('#manualDemandAudience').value:'',destination:$('#manualDemandDestination').value});appendDemandEvent(event,true);state.recentImportIds=[event.id,...state.recentImportIds].slice(0,12);persist();renderAll();switchView(event.requiresApproval?'events':'demand');toast(event.requiresApproval?`已建立 ${event.id}，因落在接單LT內已送事件審批`:`已建立需求事件 ${event.id}`);}catch(err){toast(err.message||'需求資料檢核失敗');}
}
...
function normalizeImportRow(row){
  ...
  return {source:get('source','sourceCategory','來源','需求來源'),demandType:get('demandType','type','需求類型'),sku:get('sku','成品料號','料號'),factory:get('factory','工廠','廠區'),qty:get('qty','quantity','數量','需求數量'),demandDate:get('demandDate','date','需求日期'),proposer:get('proposer','sales','業務','提出者'),proposerDept:get('proposerDept','department','提出部門'),audience:get('audience','customer','客戶','用途'),customer:get('customer','客戶'),destination:get('destination','shipTo','出貨地','目的地')};
}
...
function renderDemandImportPreview(){
  ...
  $('#demandImportPreview').innerHTML=rows.map(r=>`<tr class="${r.valid?'':'risk-row'}"><td>${r.source||'—'}${r.demandType?`<br><span class="event-id">${r.demandType}</span>`:''}</td><td>${r.sku||'—'}</td><td>${r.factory||'—'}</td><td>${fmt(r.qty)}</td><td>${r.demandDate||'—'}</td><td>${r.proposer||'—'}</td><td><span class="tag ${r.valid?'success':'danger'}">${r.valid?'通過':r.error}</span></td></tr>`).join('')||'<tr><td colspan="7">請先選擇檔案</td></tr>';
}
function commitDemandFileImport(){
  const valid=(state.importPreviewRows||[]).filter(r=>r.valid);if(!valid.length)return;const added=[];valid.forEach(r=>{const event=createImportedDemandEvent(r);appendDemandEvent(event,false);added.push(event.id);});state.recentImportIds=[...added.reverse(),...state.recentImportIds].slice(0,12);clearDemandImportPreview();persist();renderAll();switchView('demand');toast(`已匯入 ${added.length} 筆需求事件`);
}
...
function downloadDemandTemplate(){
  const sample='source,demandType,sku,factory,qty,demandDate,proposer,proposerDept,customer,destination\n客戶,客戶預測,UPS-100,TW01,5000,2026-08-15,林怡君,全球業務規劃,Formosa Data Center,台灣／台北';const blob=new Blob(['\ufeff'+sample],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='demand-import-template.csv';a.click();URL.revokeObjectURL(a.href);
}
...
function demandFlowNode(source,events,subtitle){
  ...
  return `<button type="button" class="demand-flow-node ${state.demandType===source?'active':''}" data-demand-flow-source="${source}" data-flow-node="${source}"><span class="tag ${sourceClass[source]||'neutral'}">${source}</span><strong>${count}</strong><small>需求事件</small><em>${fmt(qty)} 件・${subtitle}</em></button>`;
}
function renderDemandConversionGraph(events){
  ...
  root.querySelectorAll('[data-demand-flow-source]').forEach(node=>node.addEventListener('click',()=>{state.demandType=state.demandType===node.dataset.demandFlowSource?'all':node.dataset.demandFlowSource;renderDemand();}));
  $('#resetDemandSourceGraph')?.addEventListener('click',()=>{state.demandSource='all';state.demandType='all';renderDemand();});
...
function renderEvents(){
  ...
  $$('.event-action').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.action==='viewDemand'){state.region=e.region;state.factory=e.factory;$('#regionSelect').value=state.region;updateFactoryOptions();$('#factorySelect').value=state.factory;state.demandSource='all';state.demandType='all';state.demandStatus='all';renderAll();switchView('demand');toast(`已切換至 ${e.demandEventId} 所屬需求總覽`);return;}if(b.dataset.action==='approveLt'){const linked=state.demandEvents.find(d=>d.id===e.demandEventId);if(linked){linked.status='已確認';linked.requiresApproval=false;touchDemandEvent(linked);}e.status='已核准';e.targetDemandStatus='已確認';persist();renderAll();toast('接單LT例外已核准，需求已進入供需計畫');switchView('demand');return;}e.status=b.dataset.action;persist();renderAll();toast(`事件已更新為「${e.status}」`);if(e.status==='模擬中')switchView('simulation');}));
}
...
function persist(){saveStored('ct-events-v6-lt',state.events);saveStored('ct-demand-events-v6-lt',state.demandEvents);saveStored('ct-tracking-v3',state.tracking);saveStored('ct-commits-v3',state.commits);saveStored('ct-decisions-v3',state.decisions);}
