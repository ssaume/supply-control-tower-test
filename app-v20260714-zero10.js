...
const state={
  ...
  events:loadStored('ct-events-v7-zero',[]),
  demandEvents:loadStored('ct-demand-events-v7-zero',[]),
  tracking:loadStored('ct-tracking-v4-zero',[]),
  commits:loadStored('ct-commits-v4-zero',{}),decisions:loadStored('ct-decisions-v4-zero',{})
};
...
function setupInteractions(){
  ...
  $('#resetDemo').addEventListener('click',()=>{
    clearStored(['ct-events-v3','ct-events-v4','ct-events-v5-empty','ct-events-v6-lt','ct-events-v7-zero','ct-demand-events-v1','ct-demand-events-v2','ct-demand-events-v3','ct-demand-events-v5-empty-select','ct-demand-events-v6-lt','ct-demand-events-v7-zero','ct-tracking-v3','ct-tracking-v4-zero','ct-commits-v3','ct-commits-v4-zero','ct-decisions-v3','ct-decisions-v4-zero']);
    state.events=[];state.demandEvents=[];state.tracking=[];state.commits={};state.decisions={};state.simulated=null;state.selectedEventId='';state.selectedScenario=null;state.expandedDemandId='';state.expandedShipmentId='';state.selectedWorkOrderByEvent={};state.selectedFulfillmentByEvent={};state.routeEventId='';state.importPreviewRows=[];state.recentImportIds=[];persist();renderAll();toast('所有非主檔交易資料已清除');
  });
}
...
function createImportedDemandEvent(raw){
  ...
  const seed=hashText(`${id}|${raw.sku}|${raw.factory}|${qty}`);const stockQty=0;const wipQty=0;
  ...
  const event={id,batchId:`${sourceBatchPrefix(source)}-${raw.factory}-${demandDate.replaceAll('-','')}-${String((seed%999)+1).padStart(3,'0')}`,
    ...
    supplyAllocation:{stockQty,wipQty,warehouse:`${raw.factory}-FG倉`,lotNo:'尚無庫存批號',wipOrders:[]}
  };
  return event;
}
...
function persist(){saveStored('ct-events-v7-zero',state.events);saveStored('ct-demand-events-v7-zero',state.demandEvents);saveStored('ct-tracking-v4-zero',state.tracking);saveStored('ct-commits-v4-zero',state.commits);saveStored('ct-decisions-v4-zero',state.decisions);}
