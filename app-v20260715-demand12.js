...
/* DEMAND12: demand master entry and backend transformation rules */
function renderManualSourceOptions(reset=false){
  ...
  const people=demandProposers[type]||[];const p=$('#manualDemandProposer');if(p){const keep=p.value;p.innerHTML=people.map(x=>`<option value="${x[0]}">${x[0]}</option>`).join('');if(people.some(x=>x[0]===keep))p.value=keep;}
  if(isCustomer){$('#manualDemandAudience').innerHTML=customers.map(x=>`<option value="${x.name}">${x.name}</option>`).join('');}else{$('#manualDemandAudience').innerHTML=internalDemandPurposes.map(x=>`<option value="${x}">${x}</option>`).join('');}
  refreshManualProposerDepartment();refreshManualDestinationOptions();renderManualDemandSummary();
}
function renderManualDemandSummary(){
  ...
  $('#manualDemandSummary').innerHTML=`<div><span>需求類型</span><strong>${type}${isPo?`・${$('#manualPoCategory')?.value||'外部客戶'}`:''}</strong></div><div><span>標準接單 LT</span><strong>${a.orderLeadTime} 天</strong></div><div><span>距需求日</span><strong>${a.daysToDemand} 天</strong></div><div><span>建立狀態</span><strong>${statusForDemand(type,sku,date).status}</strong></div><p>${fmt(qty)} 件｜${rule}</p>`;
}
