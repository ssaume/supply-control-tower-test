...
const state={
  ...
  demandEvents:loadStored('ct-demand-events-v3',structuredClone(baseDemandEvents)),
...
function setupInteractions(){
  ...
  $('#resetDemo').addEventListener('click',()=>{
    clearStored(['ct-events-v3','ct-events-v4','ct-demand-events-v1','ct-demand-events-v2','ct-demand-events-v3','ct-tracking-v3','ct-commits-v3','ct-decisions-v3']);
...
const demandFlowEdges=[
  ['客戶預測','客戶PO'],['緊急詢單','客戶PO'],['內部預測','內部需求'],['內部需求','計畫庫存'],['計畫庫存','客戶PO']
];
...
function renderDemandConversionGraph(events){
  ...
  root.innerHTML=`<div class="demand-flow-scroll"><div class="demand-flow-canvas"><svg id="demandFlowSvg" class="demand-flow-svg" aria-hidden="true"></svg><section class="demand-flow-column signal"><header><small>Demand Signals</small><h3>需求訊號</h3></header>${demandFlowNode('客戶預測',events,'客戶提供的滾動預測')}${demandFlowNode('緊急詢單',events,'交期與數量可行性詢問')}${demandFlowNode('內部預測',events,'S&OP與產品規劃')}</section><section class="demand-flow-column confirmed"><header><small>Approved Internal Need</small><h3>內部確認</h3></header>${demandFlowNode('內部需求',events,'經核准的補庫或調撥需求')}</section><section class="demand-flow-column stock"><header><small>Supply Preparation</small><h3>庫存準備</h3></header>${demandFlowNode('計畫庫存',events,'成品安全庫存與預先生產')}</section><section class="demand-flow-column order"><header><small>Firm Order</small><h3>正式訂單</h3></header>${demandFlowNode('客戶PO',events,'具備正式客戶與目的地')}</section></div></div><div class="demand-flow-edge-summary">${demandFlowEdges.map(([from,to])=>`<span>${from} → ${to}<strong>${demandTransitionCount(events,from,to)}</strong></span>`).join('')}</div>`;
...
function renderDemand(){
  ...
  $('#demandEventTable').innerHTML=Object.entries(grouped).map(([factory,items])=>{
    ...
    const header=`<tr class="factory-group-row"><td colspan="11"><div><strong>${factory}｜${factoryName(factory)}</strong><span>${items.length}筆事件・${fmt(total)}件</span></div></td></tr>`;
    return header+items.map(d=>demandRow(d)+(state.expandedDemandId===d.id?`<tr class="fulfillment-expand-row"><td colspan="11">${fulfillmentPanel(d,'demand')}</td></tr>`:'')).join('');
  }).join('')||'<tr><td colspan="11">沒有符合條件的需求事件</td></tr>';
...
function persist(){saveStored('ct-events-v4',state.events);saveStored('ct-demand-events-v3',state.demandEvents);saveStored('ct-tracking-v3',state.tracking);saveStored('ct-commits-v3',state.commits);saveStored('ct-decisions-v3',state.decisions);}
