...
const customerDemandSources=['客戶預測','緊急詢單','客戶PO'];
...
const state={
  ...
  navGroups:loadStored('ct-nav-groups-v2',{input:false,overview:false,approval:false}),
  events:loadStored('ct-events-v5-empty',structuredClone(baseEvents)),
  demandEvents:loadStored('ct-demand-events-v5-empty-select',structuredClone(baseDemandEvents)),
...
const sourceClass={'客戶預測':'info','內部預測':'neutral','內部需求':'info','客戶PO':'success','計畫庫存':'warning','緊急詢單':'danger'};
const statusClass={'待整合':'neutral','待確認':'warning','已確認':'info','已轉換':'success','部分消耗':'warning','已被PO消耗':'neutral','已轉內部SO':'success','已取消':'danger'};

function init(){setupFilters();setupNavigation();setupInteractions();setupNetworkControls();setupDemandImport();renderAll();setupTour();}
...
function switchView(view,preserveOverlay=false){
  ...
  const titles={import:'需求匯入',overview:'總覽控制塔',demand:'需求總覽',shipment:'出貨總覽',events:'需求變更事件',simulation:'有限／無限模擬',network:'供應網路與 BOM',routing:'生產供應途程',production:'產區現況',bottlenecks:'瓶頸分析',commit:'責任人 Commit',decision:'跨部門決策室',tracking:'執行追蹤與 Highlight'};
...
function setupInteractions(){
  ...
  $('#resetDemo').addEventListener('click',()=>{
    clearStored(['ct-events-v3','ct-events-v4','ct-events-v5-empty','ct-demand-events-v1','ct-demand-events-v2','ct-demand-events-v3','ct-demand-events-v5-empty-select']);
    state.events=[];state.demandEvents=[];state.selectedEventId='';state.selectedScenario=null;state.expandedDemandId='';state.expandedShipmentId='';state.selectedWorkOrderByEvent={};state.selectedFulfillmentByEvent={};state.routeEventId='';state.importPreviewRows=[];state.recentImportIds=[];persist();renderAll();toast('需求事件資料已清除');
  });
}
const validDemandSources=['客戶預測','內部預測','內部需求','計畫庫存','緊急詢單','客戶PO'];
function defaultPurpose(source,audience=''){
  if(source==='客戶預測')return audience||'客戶需求預測（尚未形成正式訂單）';
  if(source==='內部預測')return audience||'S&OP內部需求預測';
  if(source==='內部需求')return audience||'經核准的內部補庫／調撥需求';
  if(source==='計畫庫存')return audience||'區域成品安全庫存';
  if(source==='緊急詢單')return audience||'緊急交期與數量可行性詢問';
  return '正式客戶訂單';
}
function sourceBatchPrefix(source){return {'客戶預測':'FCST','內部預測':'IFC','內部需求':'IDM','計畫庫存':'STK','緊急詢單':'RFQ','客戶PO':'PO'}[source]||'DEM';}
...
function createImportedDemandEvent(raw){
  ...
  const source=String(raw.source||'').trim();if(!validDemandSources.includes(source))throw new Error(`無效需求來源：${source}`);
  ...
  const id=nextDemandId();const dest=parseDestination(raw.destination||`${raw.destinationCountry||''}/${raw.destinationCity||''}`);
  const proposer=String(raw.proposer||'需求匯入使用者').trim();const proposerDept=String(raw.proposerDept||'需求管理').trim();
  const seed=hashText(`${id}|${raw.sku}|${raw.factory}|${qty}`);const stockQty=Math.round(qty*[.08,.14,.2,.26,.32][seed%5]);const wipQty=Math.min(qty-stockQty,Math.round(qty*[.18,.24,.3,.36][(seed>>3)%5]));
  const event={id,batchId:`${sourceBatchPrefix(source)}-${raw.factory}-${demandDate.replaceAll('-','')}-${String((seed%999)+1).padStart(3,'0')}`,
    parentEventId:'',rootEventId:id,childEventIds:[],originMode:'人工／檔案匯入',region:factoryLookup[raw.factory].region,factory:raw.factory,source,status:source==='客戶PO'?'待確認':'待整合',
    demandDate,productKey:item.productKey,product:item.product,sku:item.sku,qty,remainingQty:source==='計畫庫存'?qty:null,priority:source==='緊急詢單'?1:source==='客戶PO'?2:3,
    ...
    purpose:customerDemandSources.includes(source)?String(raw.note||defaultPurpose(source,'')).trim():String(raw.audience||raw.note||defaultPurpose(source,'')).trim(),customer:source==='客戶PO'?String(raw.customer||raw.audience||'待補客戶').trim():'',destinationRegion:'',destinationCountry:source==='客戶PO'?dest.country:'',destinationCity:source==='客戶PO'?dest.city:'',
    prospectCustomer:['客戶預測','緊急詢單'].includes(source)?String(raw.customer||raw.audience||'潛在客戶').trim():'',prospectRegion:'',prospectCountry:['客戶預測','緊急詢單'].includes(source)?dest.country:'',prospectCity:['客戶預測','緊急詢單'].includes(source)?dest.city:'',
    soNo:'',shippingStatus:'',incoterm:'',transportMode:'',note:String(raw.note||''),
    supplyAllocation:{stockQty,wipQty,warehouse:`${raw.factory}-FG倉`,lotNo:`LOT-${raw.factory}-${String(seed%9999).padStart(4,'0')}`,wipOrders:[]}
  };
  return event;
}
...
function legacyAudienceValues(source){
  if(customerDemandSources.includes(source))return customers.map(customer=>customer.name);
  return legacyDemandEntryOptions.purposes[source]||[defaultPurpose(source,'')];
}
function legacyDestinationValues(source){
  if(!customerDemandSources.includes(source))return ['不適用'];
  return customers.map(customer=>`${customer.country}／${customer.city}`);
}
function syncManualDepartmentFromProposer(){
  const source=$('#manualDemandSource')?.value;const proposer=$('#manualDemandProposer')?.value;const pair=(demandProposers[source]||[]).find(item=>item[0]===proposer);
  if(pair&&$('#manualDemandDept'))$('#manualDemandDept').value=pair[1];
}
function syncManualProposerFromDepartment(){
  const source=$('#manualDemandSource')?.value;const dept=$('#manualDemandDept')?.value;const pair=(demandProposers[source]||[]).find(item=>item[1]===dept);
  if(pair&&$('#manualDemandProposer'))$('#manualDemandProposer').value=pair[0];
}
function syncManualDestinationFromAudience(){
  const source=$('#manualDemandSource')?.value;if(!customerDemandSources.includes(source))return;
  const customer=customers.find(item=>item.name===$('#manualDemandAudience')?.value);
  if(customer&&$('#manualDemandDestination'))$('#manualDemandDestination').value=`${customer.country}／${customer.city}`;
}
function renderManualPresetOptions(reset=false){
  const source=$('#manualDemandSource')?.value||'客戶預測';
  const proposerSelect=$('#manualDemandProposer'),deptSelect=$('#manualDemandDept'),audienceSelect=$('#manualDemandAudience'),destinationSelect=$('#manualDemandDestination'),noteSelect=$('#manualDemandNote');
  const proposerPool=demandProposers[source]||[['系統管理員','需求治理']];
  const keepProposer=reset?'':proposerSelect?.value,keepDept=reset?'':deptSelect?.value,keepAudience=reset?'':audienceSelect?.value,keepDestination=reset?'':destinationSelect?.value,keepNote=reset?'':noteSelect?.value;
  proposerSelect.innerHTML=selectOptionsHtml(proposerPool.map(item=>item[0]));
  deptSelect.innerHTML=selectOptionsHtml([...new Set(proposerPool.map(item=>item[1]))]);
  const audiences=legacyAudienceValues(source);audienceSelect.innerHTML=selectOptionsHtml(audiences);
  const destinations=legacyDestinationValues(source);destinationSelect.innerHTML=selectOptionsHtml(destinations);
  const notes=legacyDemandEntryOptions.purposes[source]||[defaultPurpose(source,'')];noteSelect.innerHTML=selectOptionsHtml(notes);
  if(proposerPool.some(item=>item[0]===keepProposer))proposerSelect.value=keepProposer;
  if(proposerPool.some(item=>item[1]===keepDept))deptSelect.value=keepDept;else syncManualDepartmentFromProposer();
  if(audiences.includes(keepAudience))audienceSelect.value=keepAudience;
  if(destinations.includes(keepDestination))destinationSelect.value=keepDestination;else syncManualDestinationFromAudience();
  if(notes.includes(keepNote))noteSelect.value=keepNote;
  destinationSelect.disabled=!customerDemandSources.includes(source);
}
function setupDemandImport(){
  const sku=$('#manualDemandSku');if(!sku)return;
  sku.innerHTML=productCatalog.slice().sort((a,b)=>a.product.localeCompare(b.product,'zh-Hant')||a.sku.localeCompare(b.sku)).map(x=>`<option value="${x.sku}">${x.product}｜${x.sku}</option>`).join('');
  $('#manualDemandQty').innerHTML=selectOptionsHtml(legacyDemandEntryOptions.quantities,value=>`${fmt(value)} 件`);
  $('#manualDemandDate').innerHTML=selectOptionsHtml(legacyDemandEntryOptions.dates,value=>formatDateFull(value));
  $('#manualDemandQty').value=String(legacyDemandEntryOptions.quantities.find(value=>value>=5000)||legacyDemandEntryOptions.quantities[0]);
  const targetDate=shiftDate(TODAY,14);$('#manualDemandDate').value=legacyDemandEntryOptions.dates.find(value=>value>=targetDate)||legacyDemandEntryOptions.dates[0];
  const update=()=>{renderManualFactoryOptions();renderManualDemandSummary();};
  sku.addEventListener('change',update);
  $('#manualDemandSource').addEventListener('change',()=>{renderManualPresetOptions(true);renderManualDemandSummary();});
  $('#manualDemandProposer').addEventListener('change',()=>{syncManualDepartmentFromProposer();renderManualDemandSummary();});
  $('#manualDemandDept').addEventListener('change',()=>{syncManualProposerFromDepartment();renderManualDemandSummary();});
  $('#manualDemandAudience').addEventListener('change',()=>{syncManualDestinationFromAudience();renderManualDemandSummary();});
  ['manualDemandFactory','manualDemandQty','manualDemandDate','manualDemandDestination','manualDemandNote'].forEach(id=>$('#'+id)?.addEventListener('change',renderManualDemandSummary));
  $('#manualDemandForm').addEventListener('submit',e=>{e.preventDefault();saveManualDemand();});
  $('#resetManualDemand').addEventListener('click',()=>setTimeout(()=>{sku.selectedIndex=0;renderManualFactoryOptions();renderManualPresetOptions(true);$('#manualDemandQty').value=String(legacyDemandEntryOptions.quantities.find(value=>value>=5000)||legacyDemandEntryOptions.quantities[0]);$('#manualDemandDate').value=legacyDemandEntryOptions.dates.find(value=>value>=targetDate)||legacyDemandEntryOptions.dates[0];renderManualDemandSummary();},0));
  ...
  renderManualFactoryOptions();renderManualPresetOptions(true);renderManualDemandSummary();
}
function renderManualFactoryOptions(){
  ...
  select.innerHTML=factories.map(factory=>`<option value="${factory}">${factory}｜${factoryName(factory)}｜${factoryLookup[factory].regionName}</option>`).join('');
  if(factories.includes(current))select.value=current;
}
function renderManualDemandSummary(){
  const root=$('#manualDemandSummary');if(!root)return;const item=skuLookup[$('#manualDemandSku')?.value];const factory=$('#manualDemandFactory')?.value;const qty=Number($('#manualDemandQty')?.value||0);const source=$('#manualDemandSource')?.value;
  root.innerHTML=item?`<div><span>成品</span><strong>${item.product}｜${item.sku}</strong></div><div><span>可生產工廠</span><strong>${factory?plantLabel(factory):'請選擇'}</strong></div><div><span>需求來源鏈</span><strong>${source==='緊急詢單'?'緊急詢單 → 客戶PO（獨立鏈）':source==='客戶預測'?'客戶預測 → 客戶PO':source==='內部預測'?'內部預測 → 內部需求 → 計畫庫存':source==='內部需求'?'內部需求 → 計畫庫存':source==='計畫庫存'?'計畫庫存 → 客戶PO消耗':'直接客戶PO'}</strong></div><div><span>需求量／日期</span><strong>${qty?fmt(qty)+' 件':'請選擇'}・${$('#manualDemandDate')?.value?formatDateFull($('#manualDemandDate').value):'請選擇'}</strong></div><div><span>提出單位</span><strong>${$('#manualDemandProposer')?.value||'—'}｜${$('#manualDemandDept')?.value||'—'}</strong></div><div><span>客戶／用途</span><strong>${$('#manualDemandAudience')?.value||'—'}</strong></div>`:'';
}
function saveManualDemand(){
  const factory=$('#manualDemandFactory')?.value;if(!factory){toast('請選擇可生產工廠');return;}
  try{const event=createImportedDemandEvent({source:$('#manualDemandSource').value,sku:$('#manualDemandSku').value,factory,qty:$('#manualDemandQty').value,demandDate:$('#manualDemandDate').value,proposer:$('#manualDemandProposer').value,proposerDept:$('#manualDemandDept').value,audience:$('#manualDemandAudience').value,destination:$('#manualDemandDestination').value,note:$('#manualDemandNote').value});state.demandEvents.unshift(event);state.recentImportIds=[event.id,...state.recentImportIds].slice(0,12);persist();renderAll();switchView('demand');toast(`已建立需求事件 ${event.id}`);}catch(err){toast(err.message||'需求資料檢核失敗');}
}
...
function normalizeImportRow(row){
  ...
  return {source:get('source','來源','需求來源'),sku:get('sku','成品料號','料號'),factory:get('factory','工廠','廠區'),qty:get('qty','quantity','數量','需求數量'),demandDate:get('demandDate','date','需求日期'),proposer:get('proposer','提出者'),proposerDept:get('proposerDept','department','提出部門'),audience:get('audience','customer','客戶','用途'),destination:get('destination','目的地'),note:get('note','備註')};
}
...
function renderDemandImportPreview(){
  ...
  $('#demandImportPreview').innerHTML=rows.map(r=>`<tr class="${r.valid?'':'risk-row'}"><td>${r.source||'—'}</td><td>${r.sku||'—'}</td><td>${r.factory||'—'}</td><td>${fmt(r.qty)}</td><td>${r.demandDate||'—'}</td><td>${r.proposer||'—'}</td><td><span class="tag ${r.valid?'success':'danger'}">${r.valid?'通過':r.error}</span></td></tr>`).join('')||'<tr><td colspan="7">請先選擇檔案</td></tr>';
}
function commitDemandFileImport(){
  const valid=(state.importPreviewRows||[]).filter(r=>r.valid);if(!valid.length)return;const added=[];valid.forEach(r=>{const event=createImportedDemandEvent(r);state.demandEvents.push(event);added.push(event.id);});state.recentImportIds=[...added.reverse(),...state.recentImportIds].slice(0,12);clearDemandImportPreview();persist();renderAll();switchView('demand');toast(`已匯入 ${added.length} 筆需求事件`);
}
...
function renderAll(){renderContext();renderOverview();renderDemandImport();renderDemand();renderShipment();renderEvents();renderSimulation();renderNetwork();renderRouting();renderProduction();renderBottlenecks();renderCommit();renderDecision();renderTracking();}
...
function bindRouteEcosystemFocus(){
  ...
  canvas.querySelectorAll('.ecosystem-node').forEach(node=>node.addEventListener('click',()=>{
...
const demandFlowEdges=[
  ['客戶預測','客戶PO'],['內部預測','內部需求'],['內部需求','計畫庫存'],['計畫庫存','客戶PO'],['緊急詢單','客戶PO']
];
function demandFlowCount(events,source){return events.filter(e=>e.source===source).length;}
function demandTransitionCount(events,from,to){
  return events.reduce((sum,event)=>sum+(event.source===from?(event.childEventIds||[]).map(id=>state.demandEvents.find(x=>x.id===id)).filter(x=>x?.source===to).length:0),0);
}
function demandFlowNode(source,events,subtitle){
  const count=demandFlowCount(events,source);
  const qty=events.filter(e=>e.source===source).reduce((s,e)=>s+Number(e.qty||0),0);
  return `<button type="button" class="demand-flow-node ${state.demandSource===source?'active':''}" data-demand-flow-source="${source}" data-flow-node="${source}"><span class="tag ${sourceClass[source]||'neutral'}">${source}</span><strong>${count}</strong><small>需求事件</small><em>${fmt(qty)} 件・${subtitle}</em></button>`;
}
function renderDemandConversionGraph(events){
  const root=$('#demandConversionGraph');if(!root)return;
  root.innerHTML=`<div class="demand-flow-scroll"><div class="demand-flow-canvas demand-flow-canvas-v2"><svg id="demandFlowSvg" class="demand-flow-svg" aria-hidden="true"></svg><section class="demand-flow-column signal"><header><small>Normal Demand Signals</small><h3>正常需求訊號</h3></header>${demandFlowNode('客戶預測',events,'客戶提供的滾動預測')}${demandFlowNode('內部預測',events,'S&OP與產品規劃')}</section><section class="demand-flow-column confirmed"><header><small>Approved Internal Need</small><h3>內部確認</h3></header>${demandFlowNode('內部需求',events,'經核准的補庫或調撥需求')}</section><section class="demand-flow-column stock"><header><small>Supply Preparation</small><h3>庫存準備</h3></header>${demandFlowNode('計畫庫存',events,'成品安全庫存與預先生產')}</section><section class="demand-flow-column order"><header><small>Firm Order</small><h3>正式訂單</h3></header>${demandFlowNode('客戶PO',events,'具備正式客戶與目的地')}</section><section class="demand-inquiry-lane"><header><small>Independent Inquiry Flow</small><h3>詢單獨立關係鏈</h3><p>詢單不納入正常預測／內部補庫鏈，確認可行後才拉到客戶 PO。</p></header>${demandFlowNode('緊急詢單',events,'交期與數量可行性詢問')}</section></div></div><div class="demand-flow-edge-summary">${demandFlowEdges.map(([from,to])=>`<span>${from} → ${to}<strong>${demandTransitionCount(events,from,to)}</strong></span>`).join('')}</div>`;
  root.querySelectorAll('[data-demand-flow-source]').forEach(node=>node.addEventListener('click',()=>{state.demandSource=state.demandSource===node.dataset.demandFlowSource?'all':node.dataset.demandFlowSource;renderDemand();}));
  $('#resetDemandSourceGraph')?.addEventListener('click',()=>{state.demandSource='all';renderDemand();});
  requestAnimationFrame(drawDemandFlowConnections);
}
...
function renderDemand(){
  ...
  const sources=['客戶預測','緊急詢單','內部預測','內部需求','計畫庫存','客戶PO'];
  $('#demandKpis').innerHTML=sources.map((source,i)=>kpiCard([source,all.filter(d=>d.source===source).length,`${fmt(all.filter(d=>d.source===source).reduce((s,d)=>s+d.qty,0))} 件`,['F','!','IF','ID','S','PO'][i],['tone-blue','tone-red','tone-green','tone-blue','tone-yellow','tone-green'][i]])).join('');
...
function demandRow(d){
  ...
  if(d.status==='待整合')action=['送確認','confirmQueue'];
    ...
    else if(d.source==='內部預測')action=['轉內部需求','toInternalDemand'];
    else if(d.source==='內部需求')action=['轉計畫庫存','toPlannedStock'];
...
function createDerivedDemandEvent(parent,targetSource){
  ...
  const common={id,region:parent.region,factory:parent.factory,demandDate:parent.demandDate,productKey:parent.productKey,product:parent.product,sku:parent.sku,priority:parent.priority,createdDate:TODAY,parentEventId:parent.id,rootEventId:parent.rootEventId||parent.id,childEventIds:[],soNo:'',shippingStatus:'',incoterm:'',transportMode:'',supplyAllocation:null};
  let child;
  if(targetSource==='內部需求'){
    child={...common,batchId:`IDM-${parent.factory}-${String(hashText(id)%999).padStart(3,'0')}`,source:'內部需求',status:'已確認',qty:parent.qty,remainingQty:null,originMode:'內部預測轉內部需求',purpose:'經核准的內部補庫／調撥需求',customer:'',destinationRegion:'',destinationCountry:'',destinationCity:''};
    parent.status='已轉換';
  }else if(targetSource==='計畫庫存'){
    child={...common,batchId:`STK-${parent.factory}-${String(hashText(id)%999).padStart(3,'0')}`,source:'計畫庫存',status:'已確認',qty:parent.qty,remainingQty:parent.qty,originMode:'內部需求轉庫存',purpose:'區域成品安全庫存',customer:'',destinationRegion:'',destinationCountry:'',destinationCity:''};
    parent.status='已轉換';
  }else{
...
function handleDemandAction(id,action){
  ...
  if(action==='confirm'){d.status='已確認';touchDemandEvent(d);}
...
function renderEvents(){
  ...
  const e=selectedEvent();if(!e){$('#eventDetail').innerHTML='<div class="empty-state">尚無需求變更事件。請先匯入需求，並將上游需求轉為客戶 PO。</div>';return;}const demand=state.demandEvents.find(d=>d.id===e.demandEventId);const upstream=state.demandEvents.find(d=>d.id===e.upstreamEventId);const qtyDelta=e.newQty-e.oldQty;const days=dateDiff(e.newDate,e.oldDate);
...
function persist(){saveStored('ct-events-v5-empty',state.events);saveStored('ct-demand-events-v5-empty-select',state.demandEvents);saveStored('ct-tracking-v3',state.tracking);saveStored('ct-commits-v3',state.commits);saveStored('ct-decisions-v3',state.decisions);}
