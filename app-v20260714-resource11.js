...
function init(){setupFilters();setupNavigation();setupInteractions();setupNetworkControls();setupDemandImport();setupResourceManagement();renderAll();setupTour();}
...
function switchView(view,preserveOverlay=false){
  ...
  const titles={import:'需求匯入',workorders:'工單匯入',labor:'工時管理',capacity:'產能管理',overview:'總覽控制塔',demand:'需求總覽',shipment:'出貨總覽',events:'需求變更事件',simulation:'有限／無限模擬',network:'供應網路與 BOM',routing:'生產供應途程',production:'產區現況',bottlenecks:'瓶頸分析',commit:'責任人 Commit',decision:'跨部門決策室',tracking:'執行追蹤與 Highlight'};
...
function renderAll(){renderContext();renderOverview();renderDemandImport();renderWorkOrderImport();renderLaborManagement();renderCapacityManagement();renderDemand();renderShipment();renderEvents();renderSimulation();renderNetwork();renderRouting();renderProduction();renderBottlenecks();renderCommit();renderDecision();renderTracking();}
