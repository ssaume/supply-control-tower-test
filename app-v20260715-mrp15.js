...
function updateDemandMrpSummaries(){
  state.demandEvents.forEach(d=>{
    const wo=state.plannedOrders.filter(x=>x.eventId===d.id),po=state.purchaseOrders.filter(x=>x.eventId===d.id);
    d.mrpSummary={plannedOrderCount:wo.length,purchaseOrderCount:po.length,requiredHours:Number(wo.reduce((s,x)=>s+Number(x.requiredHours||0),0).toFixed(2)),capacityShortfall:Number(wo.reduce((s,x)=>s+Number(x.capacityShortfall||0),0).toFixed(2)),purchaseQty:po.reduce((s,x)=>s+Number(x.qty||0),0),levels:Math.max(0,...[...wo,...po].map(x=>Number(x.level||0))),lastRunAt:new Date().toISOString()};
  });
}
...
async function recalculateMrpAsync(reason='需求更新'){
  ...
  state.mrpBusy=false;state.mrpProgress={done:buckets.length,total:buckets.length,label:'完成'};mrpSaveSmall();renderMrpEngineStatus();return true;
}
function scheduleMrpRecalculation(reason='需求更新',after){
  ...
  setTimeout(async()=>{try{await recalculateMrpAsync(reason);renderDemand();renderWorkOrderImport();renderProduction();renderSimulation();renderBottlenecks();renderOverview();after?.();}catch(err){console.error(err);toast('MRP 計算失敗：'+err.message);}finally{if(button)button.disabled=false;}},0);
}
...
function saveManualDemand(){
  ...
  setManualDemandBusy(true);requestAnimationFrame(()=>setTimeout(()=>{try{const type=$('#manualDemandType').value,category=sourceCategoryFor(type);const event=createImportedDemandEvent({demandType:type,poCategory:$('#manualPoCategory')?.value||'',sku:$('#manualDemandSku').value,factory,qty:$('#manualDemandQty').value,demandDate:$('#manualDemandDate').value,proposer:$('#manualDemandProposer').value,proposerDept:$('#manualDemandDept').value,audience:$('#manualDemandAudience').value,customer:category==='客戶'?$('#manualDemandAudience').value:'',destination:$('#manualDemandDestination').value});appendDemandEvent(event,true);state.recentImportIds=[event.id,...state.recentImportIds].slice(0,12);mrpSaveSmall();renderDemandImport();renderDemand();renderEvents();renderOverview();toast(`已建立 ${event.id}，正在執行多階 MRP…`);scheduleMrpRecalculation(`新增需求 ${event.id}`,()=>{setManualDemandBusy(false);switchView(event.requiresApproval?'events':'demand');toast(`MRP 完成：已建立計畫工單與採購單`);});}catch(err){console.error(err);toast(err.message||'需求資料檢核失敗');setManualDemandBusy(false);}},0));
}
function commitDemandFileImport(){
  ...
  requestAnimationFrame(()=>setTimeout(()=>{try{const added=[];valid.forEach(r=>{const event=createImportedDemandEvent(r);appendDemandEvent(event,false);added.push(event.id);});state.recentImportIds=[...added.reverse(),...state.recentImportIds].slice(0,12);state.importPreviewRows=[];if($('#demandFileInput'))$('#demandFileInput').value='';mrpSaveSmall();renderDemandImport();renderDemand();renderEvents();renderOverview();scheduleMrpRecalculation(`批次匯入 ${added.length} 筆需求`,()=>{if(button)button.textContent='匯入通過資料';renderDemandImportPreview();switchView('demand');toast(`已匯入 ${added.length} 筆並完成多階 MRP`);});}catch(err){console.error(err);toast(err.message||'需求匯入失敗');if(button){button.disabled=false;button.textContent='匯入通過資料';}}},0));
}
