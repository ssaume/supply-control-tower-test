...
const state={
  ...
  demandEvents:loadStored('ct-demand-events-v4-empty',structuredClone(baseDemandEvents)),
...
function setupInteractions(){
  ...
  $('#resetDemo').addEventListener('click',()=>{
    clearStored(['ct-events-v3','ct-events-v4','ct-events-v5-empty','ct-demand-events-v1','ct-demand-events-v2','ct-demand-events-v3','ct-demand-events-v4-empty']);
...
function createImportedDemandEvent(raw){
  ...
  const event={id,batchId:`${sourceBatchPrefix(source)}-${raw.factory}-${demandDate.replaceAll('-','')}-${String((seed%999)+1).padStart(3,'0')}`,
    ...
    purpose:defaultPurpose(source,raw.audience),customer:source==='客戶PO'?String(raw.customer||raw.audience||'待補客戶').trim():'',destinationRegion:'',destinationCountry:source==='客戶PO'?dest.country:'',destinationCity:source==='客戶PO'?dest.city:'',
...
function setupDemandImport(){
  ...
  $('#manualDemandDate').value=shiftDate(TODAY,14);
  const update=()=>{renderManualFactoryOptions();renderManualDemandSummary();};
  sku.addEventListener('change',update);$('#manualDemandSource').addEventListener('change',renderManualDemandSummary);['manualDemandQty','manualDemandDate','manualDemandProposer','manualDemandDept','manualDemandAudience','manualDemandDestination'].forEach(id=>$('#'+id)?.addEventListener('input',renderManualDemandSummary));
  $('#manualDemandForm').addEventListener('submit',e=>{e.preventDefault();saveManualDemand();});
  $('#resetManualDemand').addEventListener('click',()=>setTimeout(()=>{sku.selectedIndex=0;$('#manualDemandDate').value=shiftDate(TODAY,14);renderManualFactoryOptions();renderManualDemandSummary();},0));
  ...
  renderManualFactoryOptions();renderManualDemandSummary();
}
function renderManualFactoryOptions(){
  const sku=$('#manualDemandSku')?.value;const root=$('#manualFactoryOptions');if(!root)return;const factories=availableFactoriesForSku(sku);const current=root.querySelector('input:checked')?.value;
  root.innerHTML=factories.map((factory,i)=>`<label class="factory-option"><input type="radio" name="manualFactory" value="${factory}" ${factory===current||(!current&&i===0)?'checked':''}><span><strong>${factory}｜${factoryName(factory)}</strong><small>${factoryLookup[factory].regionName}・可生產此成品</small></span></label>`).join('');
  root.querySelectorAll('input').forEach(x=>x.addEventListener('change',renderManualDemandSummary));
}
function renderManualDemandSummary(){
  const root=$('#manualDemandSummary');if(!root)return;const item=skuLookup[$('#manualDemandSku')?.value];const factory=document.querySelector('input[name="manualFactory"]:checked')?.value;const qty=Number($('#manualDemandQty')?.value||0);const source=$('#manualDemandSource')?.value;
  root.innerHTML=item?`<div><span>成品</span><strong>${item.product}｜${item.sku}</strong></div><div><span>可生產工廠</span><strong>${factory?plantLabel(factory):'請選擇'}</strong></div><div><span>需求來源鏈</span><strong>${source==='緊急詢單'?'緊急詢單 → 客戶PO（獨立鏈）':source==='客戶預測'?'客戶預測 → 客戶PO':source==='內部預測'?'內部預測 → 內部需求 → 計畫庫存':source==='內部需求'?'內部需求 → 計畫庫存':source==='計畫庫存'?'計畫庫存 → 客戶PO消耗':'直接客戶PO'}</strong></div><div><span>需求量／日期</span><strong>${qty?fmt(qty)+' 件':'待輸入'}・${$('#manualDemandDate')?.value?formatDateFull($('#manualDemandDate').value):'待輸入'}</strong></div>`:'';
}
function saveManualDemand(){
  const factory=document.querySelector('input[name="manualFactory"]:checked')?.value;if(!factory){toast('請選擇可生產工廠');return;}
...
function persist(){saveStored('ct-events-v5-empty',state.events);saveStored('ct-demand-events-v4-empty',state.demandEvents);saveStored('ct-tracking-v3',state.tracking);saveStored('ct-commits-v3',state.commits);saveStored('ct-decisions-v3',state.decisions);}
